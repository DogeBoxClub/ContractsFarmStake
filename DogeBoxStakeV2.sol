...
contract DogeBoxStakeV2 is Ownable, IERC721Receiver {
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(ERC721Enumerable _nft, DogeBox _token) { 
    nft = _nft;
    token = _token;
  }
...
*/
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      rewardmath = 400 ether * (block.timestamp - stakedAt) / 86400 ;
      earned = rewardmath / 400;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
        timestamp: uint48(block.timestamp)
